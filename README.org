...
gc_plts <- function() {
    list(
        p_mpg_dist = list(mtcars = quote(mtcars),
                          caption = "mpg vs disp",
                          width = 6, height = 4))
}

## generate example plot
gp_mpg_dist <- function(mtcars) {
...
gplt("p_mpg_dist") # generate plot
dpltR("p_mpg_dist") # display plot
gwdplt("p_mpg_dist") # write to file and show
#+end_src
