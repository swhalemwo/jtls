...
gl_clgr_objs <- function() {
    ...
    to1a <- list(a=1,b=2)
    attr(to1a, "gnrtdby") <- "kappa1a"
    to1b <- list(k=1,oo=2)
    attr(to1b, "gnrtdby") <- "kappa1b"
    to2 <- list(to1a = to1a,
                to1b = to1b,
                jj = 3)
    attr(to2, "gnrtdby") <- "kappa2"
    to3 <- list(to2= to2, ii = 4)
    attr(to3, "gnrtdby") <- "kappa3"
    to4 <- list(to3 = to3, ll = 5)
    attr(to4, "gnrtdby") <- "kappa4"

    recx("to4")
    rec_attr_query("to4") %>% rbindlist(fill = T)

    ## %>% rbindlist

    c(list(list(letter = "a", jj = 10)), map(letters[2:5], ~list(letter = .x, jj = 10)))

    ## this is what I want, idk where all the extra levels come from
    ## also goes back 
    list(list(letter = "a", jj = 10), list(letter = "b", jj = 10))
